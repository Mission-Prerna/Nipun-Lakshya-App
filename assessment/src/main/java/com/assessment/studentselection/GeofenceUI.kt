...
object GeofenceUI {
    ...
    fun locationMatchedDialogAssessmentEnd(context: Context?, logoutClicked: (CustomMessageDialog) -> Unit) {
        context?.let {
            var customDialog: CustomMessageDialog? = null
            if (customDialog == null) {
                customDialog = CustomMessageDialog(
                    it, it.getDrawable(R.drawable.ic_location),
                    it.getString(R.string.location_matched_examiner_end_description),
                    null
                )
                customDialog.setOnFinishListener {
                    logoutClicked.invoke(customDialog)
