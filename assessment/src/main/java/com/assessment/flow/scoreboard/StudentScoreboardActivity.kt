...
class StudentScoreboardActivity : BaseActivity<StudentScoreboardBinding, ScoreboardVM>() {
    ...
    private fun showHeaderView(){
        ...
        val assessorName: String = AppPreferences.getUser()?.officer_name ?: ""
        ...
        binding.header.apply {
            ...
            if (AppPreferences.getSelectedUserType()==Constants.USER_PARENT)
                bind(header, PARENT)
            else
                bind(header, STUDENT)
        }
    ...
    private fun showRelevantNipunStatusIcon(list: MutableList<ScorecardData>) {
        viewModel.isNipunLiveData.observe(this) { isNipun ->
            if (isNipun) {
        ...
        viewModel.checkNipunStatus(list)
    }
