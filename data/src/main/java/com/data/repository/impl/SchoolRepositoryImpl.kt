...
class SchoolRepositoryImpl @Inject constructor(
    ...
    override suspend fun fetchStudentStatusHistories(
        ...
        if (response.isSuccessful) {
            ...
            allStudents = modifyEntriesWihOfflineData(allStudents, cycleId)
    ...
    override suspend fun fetchSchoolStatusHistories(cycleId: Int): Result<Unit> {
        ...
        if (response.isSuccessful) {
            ...
            val data =
                modifySchoolHistoriesWihOfflineData(histories = statusHistories, cycleId = cycleId)
    ...
    override suspend fun postSchoolSubmission(cycleId: Int, udise: Long, ctx: Context): Result<Unit> {
    ...
    private suspend fun modifyEntriesWihOfflineData(
        ...
        val localEntriesMap = studentsAssessmentHistoryDao.getAllHistories(cycleId)
            .associateBy { it.id }
        // Modify the entries based on offline data & If there is no local entry, use the server entry
