...
class AssessmentsRepositoryImpl @Inject constructor(
    ...
    override suspend fun convertStatesToSubmissions(
        ...
        if (submissionCount.isNotEmpty()) {
            ...
            if (mentorDetails.actorId == TEACHER) {
                ...
                val homeScreenStats = CoroutineScope(Dispatchers.IO).async {
                    updateHomeScreenStatsForMentor(assessmentHistories)
                }
                homeScreenStats.await()
                updateVisitedStatusInSchoolList(assessmentHistories)
    ...
    private fun updateVisitedStatusInSchoolList(assessmentHistories: List<StudentAssessmentHistory>) {
        val udisesToBeMarkedVisited = assessmentHistories.map { it.udise }.toSet()
        for (udise in udisesToBeMarkedVisited){
            CoroutineScope(Dispatchers.IO).launch {
                schoolsDao.markSchoolAsVisited(udise)
            }
        }
    }
    ...
    private fun getDistinctUdiseValuesCount(studentAssessmentHistoryList: List<StudentAssessmentHistory>): Int {
        val udiseSet = studentAssessmentHistoryList.map { it.udise }.toSet()
        val visitedSchoolsUdise = schoolsDao.getUdiseForVisitedSchools().toSet()

        val commonItemsCount = udiseSet.intersect(visitedSchoolsUdise).size

        return udiseSet.size - commonItemsCount
    }

    private suspend fun updateHomeScreenStatsForMentor(assessmentHistories: List<StudentAssessmentHistory>) {
        val mentorInsights = mentorPerformanceInsightsDao.getMentorPerformanceInsights().first()

        val gradeAndAssessedMap = mutableMapOf<Int, Int>().withDefault { 0 }
        var totalStudentsAccessed = 0

        val filteredAssessmentHistories = assessmentHistories.filter { it.id.toInt() >= 0 }

        filteredAssessmentHistories.forEach {
            if (it.status != PENDING) {
                ++totalStudentsAccessed
                val gradeForCountIncrease = studentsDao.getStudentGradeById(it.id)
                gradeAndAssessedMap[gradeForCountIncrease] = gradeAndAssessedMap.getValue(gradeForCountIncrease) + 1
        ...
        val newGradesInsights = mentorInsights.gradesInsights.map { insight ->
            when (insight.type) {
        ...
        val newTotalInsights = mentorInsights.totalInsights.map { insight ->
            when (insight.type) {
                STUDENT -> {
                    val modifiedCount = totalStudentsAccessed
                    MentorInsight(
                ...
                SCHOOL -> {
                    val modifiedCount = getDistinctUdiseValuesCount(filteredAssessmentHistories)
                    MentorInsight(
                        if (modifiedCount != 0) modifiedCount + insight.count else insight.count,
                        insight.label,
                        insight.type
        ...
        val month = mentorInsights.month
        val year = mentorInsights.year
        val newUpdatedTime = System.currentTimeMillis()

        val newMentorInsights = MentorPerformanceInsightsItem(totalInsights = newTotalInsights, gradesInsights = newGradesInsights, month = month, year = year, updated_at = newUpdatedTime)
        mentorPerformanceInsightsDao.insert(newMentorInsights)
    }
