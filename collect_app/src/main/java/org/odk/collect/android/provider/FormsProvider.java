    ...
    public synchronized Uri insert(@NonNull Uri uri, ContentValues initialValues) {
        ...
        if (formsDatabaseHelper != null) {
            ...
            try {
                ...
                if (c.getCount() > 0) {
                    Timber.e("Got existed data on forms.db");
                    // already exists
                    //delete existing form and insert new
                    db.delete(FORMS_TABLE_NAME, FormsColumns.FORM_FILE_PATH,null);
            ...
            Timber.e("Forms Provider: rowId value (if -1 then error else row no.) : "+ " " + rowId);
            if (rowId > 0) {
