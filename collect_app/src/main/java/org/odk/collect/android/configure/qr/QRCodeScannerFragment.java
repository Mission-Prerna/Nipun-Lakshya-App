...
public class QRCodeScannerFragment extends BarCodeScannerFragment {
    ...
    protected void handleScanningResult(BarcodeResult result) throws IOException, DataFormatException {
        ...
        if (importSuccess) {
            ToastUtils.showLongToast(Collect1.getInstance().getAppContext().getResources().getString(R.string.successfully_imported_settings));
            startActivityAndCloseAllOthers(requireActivity(), MainMenuActivity.class);
        } else {
            ToastUtils.showLongToast(Collect1.getInstance().getAppContext().getResources().getString(R.string.invalid_qrcode));
        }
    }
