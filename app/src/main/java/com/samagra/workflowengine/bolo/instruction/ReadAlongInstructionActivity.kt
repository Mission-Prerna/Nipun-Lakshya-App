...
open class ReadAlongInstructionActivity : AppCompatActivity() {
    ...
    private fun setBroadCastFailure() {
        BroadcastActionSingleton.getInstance().liveAppAction.value =
            BroadcastAction(
                ResultsHelper.createDummyResults(startTime, bookId),
                BroadcastEvents.READ_ALONG_FAILURE
            )
        finish()
    }
...
//        Log.e(POST_HOG_LOG_TAG, "RA INS screen $properties $EVENT_RA_COMPETENCY_SELECTION")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == LAUNCH_GOOGLE_BOLO) {
            when (resultCode) {
                ...
                RESULT_CANCELED -> {
                    onBoloResultCancelled()
                }
                KeyConstants.BOOK_ID_NOT_FOUND -> {
                    setPostHogEventBookNotFound()
                    onBoloResultBookIdNotFound()
                }
                else -> {
                    showPromptAndExit()
                }
    ...
    private fun onBoloResultOk(data: Intent?){
    ...
    private fun onBoloResultCancelled(){
        showMessageDialog(getString(R.string.assessment_canceled))
    }
    private fun onBoloResultBookIdNotFound(){
    ...
    private fun showPromptAndExit() {
       showMessageDialog(getString(R.string.read_along_lang_text))
    }
