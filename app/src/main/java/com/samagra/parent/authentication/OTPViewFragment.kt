...
class OTPViewFragment : BaseSmsReceiverFragment<FragmentOtpViewPinBinding, OTPViewVM>(),
    ...
    private fun handleResendOTPSuccess(textResId: Int?) {
        hideProgressBar()
        textResId?.let { Toast.makeText(requireContext(), getString(it), Toast.LENGTH_LONG).show() }
    }
    ...
    private fun setSendOtpFunctionality(phoneNumber: String) {
        if (CommonUtilities.isNetworkAvailable(requireContext())) {
            ...
            Toast.makeText(requireContext(), getString(R.string.internet_not_connected_error_login), Toast.LENGTH_LONG).show()
        }
    }
