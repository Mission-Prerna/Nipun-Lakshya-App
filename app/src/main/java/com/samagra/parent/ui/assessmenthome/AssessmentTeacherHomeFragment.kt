...
class AssessmentTeacherHomeFragment :
    ...
    private fun setObservers() {
        ...
        lifecycleScope.launchWhenStarted {
            viewModel.syncRequiredLiveData.observe(viewLifecycleOwner) {
                callApis(enforce = it)
            }
        }
    }
    ...
    private fun confirmLogoutWithSync() {
        LogoutUI.confirmLogoutWithSync(requireContext()) {
            viewModel.syncDataToServer(prefs, {
                ...
                Toast.makeText(requireContext(), R.string.error_generic_message, Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun sendStartAssessmentEvent() {
        ...
        PostHogManager.capture(requireActivity(), EVENT_HOME_SCREEN_START_ASSESSMENT, properties)
    }
    private fun sendSchoolHistoryEvent() {
        ...
        PostHogManager.capture(requireActivity(), EVENT_HOME_SCREEN_SCHOOL_HISTORY, properties)
    }
