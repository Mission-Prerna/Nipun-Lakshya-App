...
class AssessmentSetupFragment : BaseFragment<FragmentAssessmentSetupBinding, AssessmentSetupVM>() {
    ...
    override fun getBaseViewModel(): AssessmentSetupVM {
        val viewModelProviderFactory =
            ViewModelProviderFactory(appCompatActivity!!.application, schoolsRepository)
    ...
    private fun setObservers() {
        viewModel.schoolsListLiveData.observe(viewLifecycleOwner){ schools ->
            Timber.i("list of schools: $schools")
            Timber.i("list of schools size: ${schools.size}")
            val schoolsDataList: ArrayList<SchoolsData> = schools.map { school ->
                SchoolsData(
                    udise = school.udise,
                    ...
                    nyayPanchayatId = school.nyayPanchayatId,
                    schoolLat = school.schoolLat,
                    schoolLong = school.schoolLong,
                    geofencingEnabled = school.geofencingEnabled
                )
            } as ArrayList<SchoolsData>

            districtFilterSchoolList = schoolsDataList.toMutableList() as ArrayList<SchoolsData>
            schoolList = schoolsDataList.toMutableList() as ArrayList<SchoolsData>
            mainSchoolList = schoolsDataList.toMutableList() as ArrayList<SchoolsData>

            if (schoolsDataList.isNotEmpty()) {
                districtList.clear()
                schoolsDataList.forEach {
            ...
            setRvSchoolList(schoolsDataList)
            setSpinners()
            hideProgressBar()
        }
    }
    ...
    private fun redirectToStudentSelectionScreen(schoolsData: SchoolsData?){
    ...
    * */
    ...
    private fun setPostHogEventSelectSchool(schoolsData: SchoolsData) {
        ...
        PostHogManager.capture(requireContext(), EVENT_SCHOOL_SELECTION, properties)
